...
function gameStatus(status) {
    ...
    if (status === 'start') elSmile.innerText = 'Play Again 😃'
    if (status === 'lose') elSmile.innerText = 'Play Again 🤯'
    if (status === 'win') elSmile.innerText = 'Play Again 😎'
}
...
function displayNumCell(elCell, i, j) {
    ...
    if (neighbors) {
        for (var x = 0; x < neighbors.length; x++) {
            var elCurrCell = document.getElementById(`cell-${neighbors[x].i}-${neighbors[x].j}`)
